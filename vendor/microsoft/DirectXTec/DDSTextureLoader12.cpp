...
// 
// Modified by Ludwig Füchsl
// - Getting rid of d3dx12.h
// - Getting rid of using
// - IID_PPV_ARGS instead of IID_<Name>
//--------------------------------------------------------------------------------------
...
#undef ISBITMASK
        ...
        _Outptr_ ID3D12Resource** texture) noexcept
    {
        ...
        ZeroMemory(&defaultHeapProperties, sizeof(D3D12_HEAP_PROPERTIES));

        defaultHeapProperties.Type = D3D12_HEAP_TYPE_DEFAULT;
        defaultHeapProperties.CPUPageProperty = D3D12_CPU_PAGE_PROPERTY_UNKNOWN;
        defaultHeapProperties.MemoryPoolPreference = D3D12_MEMORY_POOL_UNKNOWN;
        defaultHeapProperties.CreationNodeMask = NULL;
        defaultHeapProperties.VisibleNodeMask = NULL;
        ...
        _Out_opt_ bool* outIsCubeMap) noexcept(false)
    {
        ...
        ZeroMemory(&formatInfo, sizeof(D3D12_FEATURE_DATA_FORMAT_INFO));
        
        formatInfo.Format = format;

        if (FAILED(d3dDevice->CheckFeatureSupport(D3D12_FEATURE_FORMAT_INFO, &formatInfo, sizeof(D3D12_FEATURE_DATA_FORMAT_INFO)))) {
            return E_INVALIDARG;
        }
        ...
        if (SUCCEEDED(hr))
        {
            ...
            if (loadFlags & DirectX::DDS_LOADER_MIP_RESERVE)
            {
    ...
    //--------------------------------------------------------------------------------------
    DirectX::DDS_ALPHA_MODE GetAlphaMode(_In_ const DDS_HEADER* header) noexcept
    {
        if (header->ddspf.flags & DDS_FOURCC)
        {
